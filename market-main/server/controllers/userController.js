...
router.patch('/edit-profile/:id', async (req, res) => {
    ...
    try {
        ...
        if (checkUser && checkUser.id.toString() !== req.user._id.toString()) errors.push('This email address is already in use; ');
...
router.get('/getUserById/:id', async (req, res) => {
    try {
        ...
        let totalSells = await Product.count({
            where: {
                seller:user.id,
            }
        });
        let jsonRes = {
            ...
            totalSells: totalSells, avatar: user.avatar,
